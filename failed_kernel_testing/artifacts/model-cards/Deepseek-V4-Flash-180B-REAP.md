...
4. **Testing multiple checkpoint sizes** from 148B through 213B. The bundled package evidence preserves summary notes for the successful K160 path and selected negative/fit boundaries; it does not include every raw checkpoint-failure artifact.
...
6. **Validating the 200K needle** and selected qualitative tasks: smoke, diagrams, code, philosophy, tool calls, and long-context retrieval.

This package includes the surviving evidence records needed for handoff. Broader quality-suite or checkpoint-history claims should be verified against the original runtime repo and raw run directories before being reused as benchmark proof.
...
```text
run_dir: <spark-root>/benchmarks/deepseek-reap/single-server-sweep/k160-mtp2-200k-mnbt4096-kv6g-20260527T192208Z
...
```text
run_dir: <spark-root>/benchmarks/deepseek-reap/single-server-sweep/k160-mtp2-200k-longcoding-fixed-20260527T194241Z
...
Bundled task coverage at 200K includes smoke, ASCII, Unicode, Mermaid diagrams, code explanation, religion and philosophy prompts, tool-call fidelity, long-needle retrieval, and long-code review summaries. Smoke, diagrams, code, religion, tool calls, and needle retrieval are recorded as passing in the notes. The long-coding record is mixed and includes `passed_core: false`, so do not summarize it as a clean pass. A few qualitative rubrics missed narrow fields at 128 output tokens, so benchmark prompts should reserve more completion tokens when judging broad reasoning quality.
