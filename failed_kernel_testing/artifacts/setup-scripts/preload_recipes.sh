...
CONTROLLER_URL=${CONTROLLER_URL:-http://<spark-tailnet-ip>:8080}
API_KEY=${VLLM_STUDIO_API_KEY:-${API_KEY:-}}
INFERENCE_HOST=${INFERENCE_HOST:-<spark-tailnet-ip>}
...
recipe.host = host;
recipe.port = port;
recipe.model_path = recipe.model_path.replace("<spark-root>", sparkRoot);
...
recipe.extra_args.launch_command = recipe.extra_args.launch_command
  .replace("<repo-root>", process.env.MODEL_MODULE_DIR.replace(/\/runtime$/, ""))
  .replace("<spark-root>", sparkRoot)
  .replace("--host <spark-tailnet-ip>", `--host ${host}`)
