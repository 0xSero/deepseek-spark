...
for (const name of ["k144", "k160"]) {
  const path = join(root, "config", "recipes", `${name}.json`);
  ...
  if (!String(recipe.extra_args.launch_command ?? "").includes("/deepseek-spark/")) {
    throw new Error(`${path} must point at /home/sero/spark/deepseek-spark`);
  }
}
